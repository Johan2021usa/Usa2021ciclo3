...
function ayaxGetSimpleClient(){
	...
	dataType: 'json',
	contentType: "application/json; charset=utf-8",
	success:function(valoresGetClient){ //el succes es necesario para crear y almacenar una variable con los datos que recibe ajax en este caso
		//console.log(valoresGet); //sirve para mostrar valoresGet en consola del navegador
		graficarValoresClient(valoresGetClient) //aqui se esta enviando una variable con datos a una funcion
	},
	error : function(xhr, status) {
		alert('ha sucedido un problema ayax simple:'+ status);	
	}
...
function graficarValoresClient(valoresGetRecibidosClient){
	//console.log(valoresGetRecibidos);
	let miTabla='<table class="edicionCelda">'
	$("#contenedor3").empty(); //aqui se limpia el contenedor al inicializar la funcion
	for (i=0; i<2; i++){
		miTabla += '<tr>'; //tr significa un piso y cada td es una division del piso
		...
		miTabla += '</tr>';
		miTabla += '<tr>';
		for (i = 0; i < valoresGetRecibidosClient.items.length; i++) {
			miTabla += '<td>' + valoresGetRecibidosClient.items[i].id + '</td>';
			miTabla += '<td>' + valoresGetRecibidosClient.items[i].name + '</td>';
			miTabla += '<td>' + valoresGetRecibidosClient.items[i].email + '</td>';
			miTabla += '<td>' + valoresGetRecibidosClient.items[i].age + '</td>';
			miTabla += '<td>' + '<input type="button" value="detalle" onclick="detalleClient(' + valoresGetRecibidosClient.items[i].id + ')">' + '</td>';
			miTabla += '<td>' + '<input type="button" value="eliminar" onclick="eliminarClient(' + valoresGetRecibidosClient.items[i].id + ')">' + '</td>';
			miTabla += '</tr>';
		}
	}
	miTabla += '</table>';
	console.log(miTabla)
	$("#contenedor3").append(miTabla); //aquí se esta almacendando los valores de mi tabla en contenedor para ser mostrados en el navegador
}
...
function validarexistenciaClient(id2){
    $.ajax({    
        url : 'https://g26540496168d53-reto1basedatos.adb.sa-vinhedo-1.oraclecloudapps.com/ords/admin/client/'+id2,
        data: "{}",
        type : 'GET',
        dataType : 'json',
        contentType: "application/json; charset=utf-8",
